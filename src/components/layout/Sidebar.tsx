...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-60 h-screen bg-base border-r border-white/[0.04] shrink-0">
      {/* Logo */}
      ...
      {/* Spotlight trigger — glass element */}
      <button
        onClick={() => useSpotlightStore.getState().open()}
        className="mx-3 mb-4 flex items-center gap-2 h-8 px-3 rounded-lg bg-white/[0.03] border border-white/[0.06] text-text-tertiary hover:text-text-secondary hover:bg-white/[0.06] transition-colors w-[calc(100%-24px)]"
      >
        ...
        <kbd className="text-[10px] font-mono bg-white/[0.06] border border-white/[0.04] px-1.5 py-0.5 rounded">⌘K</kbd>
      </button>
      ...
      <nav className="flex-1 px-3 py-2 flex flex-col gap-0.5">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn(
                "relative flex items-center gap-3 h-9 px-3 rounded-lg w-full text-label transition-colors duration-150",
                isActive
              ...
              <>
                {isActive && (
                  <motion.div
                    ...
                    className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-cyan-primary rounded-full"
      ...
      <div className="px-3 py-4 border-t border-white/[0.04]">
        {isConnected && evmAddress ? (
          ...
          <button
            ...
            className="w-full h-10 rounded-lg bg-cyan-primary text-text-on-cyan text-label font-medium hover:bg-cyan-hover transition-colors"
          >
            Connect Wallet
          </button>
        )}
