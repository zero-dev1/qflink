      ...
      >
        <div className="space-y-3">
          ...
          {linkedWallets.length === 0 ? (
            <p className="text-sm text-qx-text-muted py-4 text-center">No linked wallets</p>
          ) : (
            linkedWallets.map((wallet) => (
              <div key={wallet.address} className="flex items-center gap-3 border border-gray-200 dark:border-gray-800 bg-transparent p-3">
                <Avatar address={wallet.address} size="sm" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-qx-text-primary truncate">{truncateAddress(wallet.address)}</p>
          ...
          <div className="flex items-center justify-between bg-qx-elevated p-3">
            <span className="text-sm text-qx-text-secondary">Total Aggregated Balance</span>
            ...
            {NETWORK_ORDER.map((id) => {
              ...
              return (
                ...
                >
                  ...
                  {net.description && (
                    <p className={`text-[10px] mt-1 ${activeNetwork === id ? 'text-white/70 dark:text-black/70' : 'text-qx-text-muted'}`}>{net.description}</p>
                  )}
                </button>
              )
            })}
      ...
      <Card header={{ title: 'Encryption Keys' }}>
        <div className="space-y-3">
          {encryptionKeyPair ? (
            <>
              <div>
                <label className="text-xs text-qx-text-muted">Public Key</label>
                <div className="mt-1 flex items-center gap-2">
                  <code className="flex-1 truncate rounded bg-qx-elevated px-3 py-2 text-xs text-qx-text-secondary">
            ...
            <div className="text-center py-4">
              <p className="text-sm text-qx-text-muted mb-3">No encryption keys generated</p>
      ...
      <Card header={{ title: 'About' }}>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-qx-text-primary">QFLink v1.0.0</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm text-qx-text-secondary">Network</span>
            <span className="text-sm font-medium text-qx-text-primary">QF Network</span>
          </div>
